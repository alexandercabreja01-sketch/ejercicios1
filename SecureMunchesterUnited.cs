...
{
    public string GetDisplayName(TeamSupport staff)
    {
        if (staff is not Staff s)
        {
            return "Too Important for a Security Pass";
        }

        var displayName = s.Title;

        if (staff is Security &&
            staff is not SecurityJunior &&
            staff is not SecurityIntern &&
            staff is not PoliceLiaison)
        {
